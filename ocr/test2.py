...
def extract_team_data_simple(zone, team_name):
    ...
    for idx, (y1, y2) in enumerate(row_groups[1:], start=1):
        ...
        stats_zero = all(player_data[k] in ["0", "0/0", ""] for k in stats_keys)
        # Vérifie si le nom est vide ou ressemble à un faux positif (1-2 lettres seulement)
        name_suspicious = len(player_data["player"].strip()) <= 2
...
def main(img_path, save_csv=True):
    """
    OCR complet sur une image NBA2K, retourne un DataFrame
    img_path : chemin vers l'image
    save_csv : si True, sauvegarde le CSV
    """
    ...
    filtered_df = df[
        ~(
            (df["player"].str.strip().str.len() <= 2)
            & (df[stats_keys].applymap(lambda x: x in ["0", "0/0", ""]).all(axis=1))
        )
    ].copy()

    print(f"\n=== Résultats finaux ({len(filtered_df)} joueurs) ===")
    print(filtered_df)

    # Ajout du score
    filtered_df["team1_name"] = team1_name
    filtered_df["team1_score"] = scores["team1_score"]
    filtered_df["team2_name"] = team2_name
    filtered_df["team2_score"] = scores["team2_score"]

    # Sauvegarde CSV
    if save_csv:
        filtered_df.to_csv("nba2k_stats.csv", index=False)
...
#     img = cv2.imread("../data/images/28786.png")
#     if img is None:
